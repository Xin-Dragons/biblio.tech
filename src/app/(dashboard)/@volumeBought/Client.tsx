...
export function Client({ wallets: initialWallets }: { wallets: WalletStat[] }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        setLoading(true)
        const wallets = await getWallets("volume_bought", period)
        setWallets(wallets)
      } finally {
        setLoading(false)
      }
                ...
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Typography color="primary" fontWeight="bold">
                      ◎
                      {period === "ONE_DAY"
                        ? wallet.volume_bought_1day?.toLocaleString(undefined, { maximumFractionDigits: 0 })
                        : bigNumberFormatter.format(wallet.volume_bought || 0)}
                    </Typography>
                  </TableCell>
                </TableRow>
              ))}
