...
export function getTokenOwnedEscrowCodec(): Codec<TokenOwnedEscrowArgs, TokenOwnedEscrow> {
  return combineCodec(getTokenOwnedEscrowEncoder(), getTokenOwnedEscrowDecoder())
}
...
): Account<TokenOwnedEscrow, TAddress> | MaybeAccount<TokenOwnedEscrow, TAddress> {
...
export async function fetchMaybeTokenOwnedEscrow<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
...
export async function fetchAllTokenOwnedEscrow(
  ...
  const maybeAccounts = await fetchAllMaybeTokenOwnedEscrow(rpc, addresses, config)
...
export async function fetchAllMaybeTokenOwnedEscrow(
  ...
  return maybeAccounts.map((maybeAccount) => decodeTokenOwnedEscrow(maybeAccount))
}
