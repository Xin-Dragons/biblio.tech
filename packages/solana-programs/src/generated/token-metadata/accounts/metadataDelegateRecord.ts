...
> {
  return combineCodec(getMetadataDelegateRecordEncoder(), getMetadataDelegateRecordDecoder())
}
...
): Account<MetadataDelegateRecord, TAddress> | MaybeAccount<MetadataDelegateRecord, TAddress> {
...
export async function fetchMetadataDelegateRecord<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
): Promise<Account<MetadataDelegateRecord, TAddress>> {
  const maybeAccount = await fetchMaybeMetadataDelegateRecord(rpc, address, config)
  assertAccountExists(maybeAccount)
  return maybeAccount
}

export async function fetchMaybeMetadataDelegateRecord<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
...
export async function fetchAllMetadataDelegateRecord(
  ...
  const maybeAccounts = await fetchAllMaybeMetadataDelegateRecord(rpc, addresses, config)
...
export async function fetchAllMaybeMetadataDelegateRecord(
  ...
  return maybeAccounts.map((maybeAccount) => decodeMetadataDelegateRecord(maybeAccount))
}
