...
> {
  return combineCodec(getHolderDelegateRecordEncoder(), getHolderDelegateRecordDecoder())
}
...
): Account<HolderDelegateRecord, TAddress> | MaybeAccount<HolderDelegateRecord, TAddress> {
...
export async function fetchHolderDelegateRecord<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
): Promise<Account<HolderDelegateRecord, TAddress>> {
  const maybeAccount = await fetchMaybeHolderDelegateRecord(rpc, address, config)
  assertAccountExists(maybeAccount)
  return maybeAccount
}

export async function fetchMaybeHolderDelegateRecord<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
...
export async function fetchAllHolderDelegateRecord(
  ...
  const maybeAccounts = await fetchAllMaybeHolderDelegateRecord(rpc, addresses, config)
...
export async function fetchAllMaybeHolderDelegateRecord(
  ...
  return maybeAccounts.map((maybeAccount) => decodeHolderDelegateRecord(maybeAccount))
}
