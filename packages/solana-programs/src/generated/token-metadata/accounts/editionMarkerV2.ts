...
export function getEditionMarkerV2Codec(): Codec<EditionMarkerV2Args, EditionMarkerV2> {
...
): Account<EditionMarkerV2, TAddress> | MaybeAccount<EditionMarkerV2, TAddress> {
  return decodeAccount(encodedAccount as MaybeEncodedAccount<TAddress>, getEditionMarkerV2Decoder())
}
...
export async function fetchMaybeEditionMarkerV2<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
...
export async function fetchAllEditionMarkerV2(
  ...
  const maybeAccounts = await fetchAllMaybeEditionMarkerV2(rpc, addresses, config)
...
export async function fetchAllMaybeEditionMarkerV2(
  ...
  return maybeAccounts.map((maybeAccount) => decodeEditionMarkerV2(maybeAccount))
}
