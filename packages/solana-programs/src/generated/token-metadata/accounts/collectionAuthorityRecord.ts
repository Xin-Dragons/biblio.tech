...
export type CollectionAuthorityRecord = { key: Key; bump: number; updateAuthority: Option<Address> }
...
> {
  return combineCodec(getCollectionAuthorityRecordEncoder(), getCollectionAuthorityRecordDecoder())
}

export function decodeCollectionAuthorityRecord<TAddress extends string = string>(
...
export async function fetchCollectionAuthorityRecord<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
): Promise<Account<CollectionAuthorityRecord, TAddress>> {
  const maybeAccount = await fetchMaybeCollectionAuthorityRecord(rpc, address, config)
  assertAccountExists(maybeAccount)
  return maybeAccount
}

export async function fetchMaybeCollectionAuthorityRecord<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
...
export async function fetchAllCollectionAuthorityRecord(
  ...
  const maybeAccounts = await fetchAllMaybeCollectionAuthorityRecord(rpc, addresses, config)
...
export async function fetchAllMaybeCollectionAuthorityRecord(
  ...
  return maybeAccounts.map((maybeAccount) => decodeCollectionAuthorityRecord(maybeAccount))
}
