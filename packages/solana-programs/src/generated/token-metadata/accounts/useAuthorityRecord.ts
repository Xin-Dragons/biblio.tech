...
export type UseAuthorityRecord = { key: Key; allowedUses: bigint; bump: number }

export type UseAuthorityRecordArgs = { key: KeyArgs; allowedUses: number | bigint; bump: number }
...
export function getUseAuthorityRecordCodec(): FixedSizeCodec<
  ...
  UseAuthorityRecord
> {
  return combineCodec(getUseAuthorityRecordEncoder(), getUseAuthorityRecordDecoder())
}
...
): Account<UseAuthorityRecord, TAddress> | MaybeAccount<UseAuthorityRecord, TAddress> {
...
export async function fetchMaybeUseAuthorityRecord<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
...
export async function fetchAllUseAuthorityRecord(
  ...
  const maybeAccounts = await fetchAllMaybeUseAuthorityRecord(rpc, addresses, config)
...
export async function fetchAllMaybeUseAuthorityRecord(
  ...
  return maybeAccounts.map((maybeAccount) => decodeUseAuthorityRecord(maybeAccount))
}
