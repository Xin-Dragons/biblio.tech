...
export function getUpdatePrimarySaleHappenedViaTokenDiscriminatorBytes() {
  return getU8Encoder().encode(UPDATE_PRIMARY_SALE_HAPPENED_VIA_TOKEN_DISCRIMINATOR)
}
...
export type UpdatePrimarySaleHappenedViaTokenInstructionData = { discriminator: number }
...
>(
  input: UpdatePrimarySaleHappenedViaTokenInput<TAccountMetadata, TAccountOwner, TAccountToken>,
...
>(
  ...
  return {
    ...
    accounts: { metadata: getNextAccount(), owner: getNextAccount(), token: getNextAccount() },
    data: getUpdatePrimarySaleHappenedViaTokenInstructionDataDecoder().decode(instruction.data),
  }
}
